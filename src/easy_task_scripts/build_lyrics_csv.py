...
def safe_stem(name: str) -> str:
    # make a filesystem-safe stem for saving transcripts
    s = Path(name).stem
    s = re.sub(r"[^\w\-\. ]+", "_", s)
...
def find_audio(filename: str, audio_roots: list[Path]) -> Optional[Path]:
    # Try exact path under roots
    for root in audio_roots:
        cand = root / filename
        if cand.exists() and cand.is_file():
            return cand

    # Fallback: search within roots
    for root in audio_roots:
        if root.exists():
...
def main():
    ...
    ap.add_argument("--audio_dir", action="append",
                    default=["data/audio"], help="Root folder(s) to search for audio. Can pass multiple.")
    ...
    for idx, row in df.iterrows():
        ...
        # Map language values (you can extend this)
        whisper_lang = None
